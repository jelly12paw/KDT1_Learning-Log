...
```py
todos_d = Todo.objects.order_by('-deadline')
```

4. 모든 데이터를 priority가 높은 순으로 정렬해서 가져오기.

```py
todo_p = Todo.objects.order_by('-priority')
```

5. priority가 5인 모든 데이터를 id를 기준으로 오름차순으로 정렬해서 가져오기.

```py
todo_p5_id = Todo.objects.filter(priority=5).order_by('id')
```

6. completed가 True인 모든 데이터를 id를 기준으로 오름차순으로 정렬해서 가져오기.

```py
todo_ct = Todo.objects.filter(completed='True').order_by('id')
```
...
```py
todo_10 = Todo.objects.get(id=10)
todo_10.priority = 5
todo_10.save()
```